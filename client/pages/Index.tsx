...
import { generateQuestions, fetchOnce } from "@/lib/endpoints";
...
export default function Index() {
  ...
  const runSubmit = async (fArg?: File | null, qArg?: string) => {
    ...
    try {
      setLoading(true);
      // Prefer the dedicated, faster endpoint with timeout
      const direct = await generateQuestions(form);
      if (!direct.success) {
        // Fallback once via proxy to maximize success rate
        ...
        setResult(String(direct.result ?? "").trim());
      }
    } finally {
      setLoading(false);
    }
