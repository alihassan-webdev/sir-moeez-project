...
// Dedicated helper for question generation to use the faster, validated server route
// Uses AbortController with a 55s timeout and returns a consistent shape
export async function generateQuestions(form: FormData): Promise<{ success: boolean; result?: string; message?: string }>
{
  const ts = Date.now();
  const url = `/api/generate-questions?t=${ts}`;
  try {
    ...
    if (!res.ok) {
      // Try to extract upstream message if available
      let message = "Server busy, please try again.";
      try {
    ...
    if (ct.includes("application/json")) {
      const j = await res.json();
      const text = typeof j === "string" ? j : (j?.result ?? j?.questions ?? j?.message ?? "");
      return { success: true, result: String(text ?? "").trim() };
    }
